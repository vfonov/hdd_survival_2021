...
@model function survival_loglogistic(log_time::Vector{Float64}, event::Vector{Bool}, hdd_model::Vector{Int64})
    ...
    σ2_bar ~ Exponential()
    # priors
    μ ~ filldist(Normal(μ_bar, σ_bar), n_model)
	# prior scale
	θ ~ filldist(LogNormal(θ_bar, σ2_bar),n_model) # truncated(Normal(0.0,5.0),0.0,Inf) MvLogNormal ?
    # fitting data
...
plot( sim_loglogistic_prior, x=:times, y=:surv, ymin=:surv_p1,ymax=:surv_p2, Geom.line, Geom.ribbon, alpha=[0.6],
        ...
        SVG("hdd_multi_loglogistic_prior_ml.svg", 15cm, 15cm)
...
sim_loglogistic_post=vcat( ( 
    insertcols!( simulate_survival_loglogistic(sample(resetrange(chain_loglogistic), 1000), μ="μ[$i]",θ="θ[$i]"), 
                 :model=> levels(data.model)[i])
    for i in 1:n_model)...)
...
plot( l1, l2, l3,
    ...
    SVG("hdd_multi_loglogistic_posterior_full_ml.svg", 15cm, 15cm)
