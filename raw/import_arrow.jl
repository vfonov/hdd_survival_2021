...
function normalize!(df)
    for f in setdiff(names(df),fields)
...
function process_table(t)
    df0=CSV.read(t, DataFrame)
    normalize!(df0)
end

function load_data(d)
    for d in filter(x->startswith(x,"data_Q1_2021") && endswith(x,".zip"),readdir(d))
        println(d)
        z=nothing
        df=nothing
        onm = rsplit(d,".")[1]
        ...
        try
            z=ZipFile.Reader(d)
            parts = Tables.partitioner(process_table, filter(x->!startswith(x.name, "__") && endswith(x.name, ".csv"), z.files) )
            Arrow.write("$(pfx)/$(onm).arrow", parts;compress=:zstd)
        finally 
            isnothing(z) || close(z)
        end
        println(" ")
